...
function unlockAccount() {
    ...
    let e = web3.personal.listAccounts;
    console.log(e);
    g_from_account = e[0];
...
async function get_instance(artifact_path) {
    ...
    assert(instance, "instance is empty");
    return instance;
}
...
async function getBookBalance(acc_address, bookkeepingVar = g_bookKeepingAbi) {
    ...
    let encode = Web3EthAbi.encodeFunctionCall(bookkeepingVar, [acc_address]);
    ...
    balance += web3._extend.utils.toBigNumber(bal);
...
async function exec_callPayFun(call, cand_bookkeeping) {
    ...
    try {
        ...
        if (call.param) {
            transactionConfig['data'] = Web3EthAbi.encodeFunctionCall(call.abi, call.param);
        }
